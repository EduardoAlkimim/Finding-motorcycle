...
import { ClipboardList, Map, Bell, AlertTriangle, CheckCircle2, Clock, X } from 'lucide-react';
...
const TIPO_CONFIG = {
  ...
  ATRASO:              { label: 'Atraso',              cor: 'bg-yellow-50 border-yellow-200', iconCor: 'text-yellow-600' },
};

export default function DespachangeLertas() {
      ...
      <div className="bg-white border-b border-gray-100 px-5 py-3 flex items-center justify-between flex-shrink-0">
        <div>
          ...
          <p className="text-xs text-gray-400">
            {naoLidos.length > 0 ? `${naoLidos.length} alerta${naoLidos.length > 1 ? 's' : ''} não lido${naoLidos.length > 1 ? 's' : ''}` : 'Tudo em ordem'}
          </p>
        </div>
        {naoLidos.length > 0 && (
